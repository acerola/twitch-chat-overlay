...
const helperTextClassName = "m-0 text-[13px] leading-[1.55] text-[rgba(255,245,248,0.72)]";
...
const selectedChoiceClassName = "border-[rgba(255,214,226,0.58)] bg-white/[0.08] -translate-y-px";
...
export function CustomizerPage({ appBaseUrl, initialConfig }: CustomizerPageProps) {
  const [draftConfig, setDraftConfig] = useState<OverlayStyleConfig>(initialConfig);
  const [generatedUrl, setGeneratedUrl] = useState(() => buildOverlayUrl(appBaseUrl, initialConfig));
  const [copyLabel, setCopyLabel] = useState("コピー");

  const previewConfig = useDeferredValue(draftConfig);
  const previewUrl = useMemo(() => buildOverlayPreviewUrl(appBaseUrl, previewConfig), [appBaseUrl, previewConfig]);
  const avatarPreviewStyle = useMemo(() => createOverlayStyleVars(draftConfig) as CSSProperties, [draftConfig]);
  ...
  return (
    ...
    >
      <div className="mx-auto grid w-[min(1320px,100%)] grid-cols-1 items-start gap-[22px] min-[1081px]:grid-cols-[minmax(320px,430px)_minmax(0,1fr)]">
        ...
        >
          <div>
            <p className="mb-[6px] text-xs font-medium uppercase tracking-[0.16em] text-[#ffd4e0]">Overlay Customizer</p>
            <h1 className="m-0 text-[clamp(30px,4vw,42px)] leading-[1.1] text-[#fff6f8]">オーバーレイを整える</h1>
            <p className={helperTextClassName}>
              チャンネル名とデバッグ設定は `.env` 前提です。見た目設定だけを URL にまとめて持たせます。
              右側のプレビューを見ながら整えて、最後に OBS 用 URL を生成します。
            </p>
          </div>

          <div className={sectionClassName}>
            <div className={sectionHeadClassName}>
              <h2 className="m-0 text-[17px] font-medium text-[#fff6f8]">フォント</h2>
              <p className={helperTextClassName}>丸文字系を中心に、配信で読みやすい 4 種だけに絞ります。</p>
            </div>
            <div className="grid grid-cols-1 gap-[10px] min-[721px]:grid-cols-2">
              {FONT_PRESET_OPTIONS.map((option) => (
                <button
                  ...
                  onClick={() => setDraftConfig((current) => ({ ...current, f: option.id }))}
                >
                  <span className="text-sm font-medium text-[#fff7fa]">{option.label}</span>
                  <span className="text-lg leading-[1.35] text-[rgba(255,241,246,0.84)]" style={{ fontFamily: option.fontFamily }}>
                    {option.previewText}
                  </span>
                </button>
              ))}
          ...
          <div className={sectionClassName}>
            <div className={sectionHeadClassName}>
              <h2 className="m-0 text-[17px] font-medium text-[#fff6f8]">メインカラー</h2>
              <p className={helperTextClassName}>花・ネームピル・バッジ配色はこの 1 色から自動で組み立てます。</p>
            </div>
            <div className="flex items-center gap-3">
              ...
              <div className="inline-flex items-center gap-[10px] rounded-2xl bg-white/[0.06] px-[14px] py-3 text-[#fff7fa]">
                ...
                />
                <code className="text-[13px]">{formatAccentColor(draftConfig.c)}</code>
              </div>
            </div>
          </div>
          ...
          <div className={sectionClassName}>
            <div className={sectionHeadClassName}>
              <h2 className="m-0 text-[17px] font-medium text-[#fff6f8]">プロフィールアイコン</h2>
              <p className={helperTextClassName}>アップロードは使わず、共有しやすいプリセットだけで完結させます。</p>
            </div>
            <div className="grid grid-cols-1 gap-[10px] min-[721px]:grid-cols-2">
              {AVATAR_PRESET_OPTIONS.map((option) => (
                <button
                  key={option.id}
                  type="button"
                  className={getAvatarChoiceClassName(draftConfig.a === option.id)}
                  onClick={() => setDraftConfig((current) => ({ ...current, a: option.id }))}
                >
                  <span
                  ...
                  <span className="text-sm font-medium text-[#fff7fa]">{option.label}</span>
                  <span className={helperTextClassName}>{option.description}</span>
                </button>
              ))}
            </div>
          </div>
          ...
          <div className={sectionClassName}>
            <div className={sectionHeadClassName}>
              <h2 className="m-0 text-[17px] font-medium text-[#fff6f8]">OBS 用 URL</h2>
              <p className={helperTextClassName}>
                保存はしないので、ここで生成した URL をそのまま OBS Browser Source に貼ります。
              </p>
            </div>
            <div className="flex flex-wrap gap-[10px]">
              ...
              <button type="button" className={secondaryButtonClassName} onClick={onCopyUrl} disabled={!generatedUrl}>
                {copyLabel}
              </button>
              <button type="button" className={secondaryButtonClassName} onClick={onResetTheme}>
                デフォルトへ戻す
              </button>
            </div>
            <textarea
        ...
        >
          <div className="flex flex-col gap-3 min-[721px]:flex-row min-[721px]:items-start min-[721px]:justify-between">
            <div>
              <p className="mb-[6px] text-xs font-medium uppercase tracking-[0.16em] text-[#ffd4e0]">Live Preview</p>
              <h2 className="m-0 leading-[1.1] text-[#fff6f8]">最終表示に近いプレビュー</h2>
            </div>
            <p className={helperTextClassName}>プレビューは実際のオーバーレイを `test=1` で埋め込み表示しています。</p>
          </div>
          <div
